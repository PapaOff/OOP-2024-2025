...
##  **Основни git команди**
 ...
 ```
git clone <URL> // Клонира (копира) съществуващо хранилище
 ```

 ```
git status     // Показва статуса на файловете (променени, нови, готови за commit и т.н.)
 ```
...
git log        // Показва историята на комитите
 ...
 ```
git add <име_на_файл>  // Добавя файл за commit (. означава "всички файлове")
git add .              // Добавя всички файлове за commit
 ```

 ```
git commit -m "Съобщение за commit"   // Запазва промените в локалното хранилище
 ```

 ```
git branch                            // Показва всички локални клонове
 ```

 ```
git branch <име_на_клон>              // Създава нов клон
 ```

 ```
git checkout <име_на_клон>           // Превключва към друг клон
git switch <име_на_клон>             // Switch е алтернативен и по-модерен начин за превключване към друг клон

 ```

 ```
git pull origin <име_на_клон>        // Изтегля и слива последните промени. Така актуализираме локалния клон в нашето repo с последните промени по клона в remote repo-то
 ```

 ```
git push origin <име_на_клон>        // Качва локални промени към remote repo
 ```
